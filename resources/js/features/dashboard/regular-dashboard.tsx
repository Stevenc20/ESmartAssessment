...
import { ArrowRight, BookOpen, Clock, GraduationCap, Sparkles, Star, TrendingUp, Trophy, Users, Zap } from 'lucide-react';
import type { Auth } from '@/types';

type PageProps = { auth: Auth };

const stats = [
    { title: 'Total Siswa',       value: '128',  change: '+12%', icon: Users,        color: '#436391', bgColor: 'bg-white' },
    { title: 'Assessment Selesai',value: '45',   change: '+8%',  icon: GraduationCap,color: '#7c3aed', bgColor: 'bg-white' },
    { title: 'Challenge Aktif',   value: '12',   change: '+3',   icon: Trophy,        color: '#d97706', bgColor: 'bg-white' },
    { title: 'Rata-rata Nilai',   value: '82.5', change: '+2.3', icon: TrendingUp,    color: '#059669', bgColor: 'bg-white' },
];

const recentActivities = [
    { id: 1, user: 'Ahmad Rizki',     action: 'Menyelesaikan Assessment', detail: 'Pemrograman Web - Bagian 3',      time: '5 menit lalu',  icon: GraduationCap },
    { id: 2, user: 'Siti Nurhaliza', action: 'Mengirim Portfolio',        detail: 'Project Akhir - E-Commerce App', time: '15 menit lalu', icon: BookOpen      },
    { id: 3, user: 'Budi Santoso',   action: 'Menyelesaikan Challenge',   detail: 'Algorithm Challenge #12',        time: '30 menit lalu', icon: Trophy        },
    { id: 4, user: 'Dewi Lestari',   action: 'Mendapatkan Badge',         detail: 'Fast Learner Badge',             time: '1 jam lalu',    icon: Star          },
    { id: 5, user: 'Fajar Nugroho',  action: 'Memulai Learning Path',     detail: 'Data Science Fundamentals',      time: '2 jam lalu',    icon: BookOpen      },
];
        ...
        <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
            ...
            <div className="relative overflow-hidden rounded-xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
                <div className="absolute inset-0 opacity-[0.04]"
                    style={{ backgroundImage: 'radial-gradient(circle, #436391 1px, transparent 1px)', backgroundSize: '24px 24px' }} />
                <div className="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div className="flex items-center gap-4">
                        <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl text-xl font-bold text-white shadow-sm"
                            style={{ background: 'linear-gradient(135deg, #436391 0%, #5a7aaa 100%)' }}>
                            {initials}
                        </div>
                        <div>
                    ...
                    <div className="flex shrink-0 flex-wrap gap-2">
                        <Link href="/assessment/create">
                            <button className="inline-flex items-center gap-2 rounded-lg px-4 py-2.5 text-sm font-bold text-white shadow-sm transition-all hover:-translate-y-0.5"
                                style={{ background: 'linear-gradient(135deg, #436391 0%, #2d4f7a 100%)' }}>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {stats.map((stat) => (
                    <div key={stat.title} className={`${stat.bgColor} group relative overflow-hidden rounded-xl border border-slate-200 p-5 transition-all hover:border-slate-300 hover:shadow-md`}>
                        ...
                        <p className="mt-1 text-xs text-slate-500">
                            <span className="font-bold text-emerald-600">{stat.change}</span> dari bulan lalu
                        </p>
                    </div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
                ...
                <div className="rounded-xl border border-slate-200 bg-white lg:col-span-2">
                    <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
                        ...
                        <Link href="/analytics" className="inline-flex items-center gap-1 text-xs font-semibold text-blue-600 hover:text-blue-700">
