...
export default function AssessmentEdit({ assessment, materiList }: { assessment: AssessmentItem; materiList: MateriItem[] }) {
    const { errors } = usePage().props;
    const { data, setData, put, processing } = useForm({
        materi_id: String(assessment.materi_id),
        judul: assessment.judul,
        deskripsi: assessment.deskripsi ?? '',
        deadline: assessment.deadline ? assessment.deadline.slice(0, 16) : '',
        bobot: String(assessment.bobot),
        max_revisi: String(assessment.max_revisi),
    });

    function submit(e: React.FormEvent) {
        e.preventDefault();
        put(`/assessment/${assessment.id}`, { preserveScroll: true });
    }
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-4 md:p-6 lg:p-8">
                <div className="mx-auto flex w-full max-w-3xl flex-col gap-6">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl font-bold text-slate-900">Edit Assessment</h1>
                            <p className="text-sm text-slate-500">{assessment.judul}</p>
                        </div>
                    </div>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <form onSubmit={submit} className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <Label>Bobot</Label>
                                    <Input type="number" min="1" max="100" value={data.bobot} onChange={e => setData('bobot', e.target.value)} />
                                    {errors.bobot && <p className="mt-1 text-sm text-red-500">{errors.bobot}</p>}
                                </div>

                                <div>
                                    <Label>Max Revisi</Label>
                                    <Input type="number" min="0" value={data.max_revisi} onChange={e => setData('max_revisi', e.target.value)} />
