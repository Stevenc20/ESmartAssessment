...
type AuditLog = {
    ...
    description: string | null; subject_type: string | null;
    created_at: string; ip_address: string | null;
};

type PageProps = {
    logs: { data: AuditLog[]; current_page: number; last_page: number; per_page: number; total: number };
    actions: string[];
};
...
export default function AuditLogsIndex({ logs, actions }: PageProps) {
    ...
    function handleSearch(val: string) {
        router.get('/admin/audit-logs', { search: val, action: actionFilter }, { preserveScroll: true, preserveState: true });
    }

    function handleAction(val: string) {
        router.get('/admin/audit-logs', { action: val, search }, { preserveScroll: true, preserveState: true });
    }
        ...
        <>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 lg:p-8">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="mt-0.5 text-sm text-slate-500">
                                    Total <span className="font-semibold text-slate-700">{logs.total}</span> entri tercatat
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex flex-col gap-3 sm:flex-row">
                    ...
                    <select value={actionFilter} onChange={e => handleAction(e.target.value)}
                    ...
                    <CardContent className="p-0">
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {logs.data.map((log) => (
                                        <tr key={log.id} className="border-b border-slate-100 last:border-0 transition-colors hover:bg-slate-50">
                                            <td className="px-5 py-3 font-semibold text-slate-900">{log.user_name}</td>
                                            <td className="px-5 py-3">
                                                <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wider ${
                                                    actionStyles[log.action] ?? 'bg-slate-100 text-slate-700'
                                                }`}>
                                                    {log.action}
                                                </span>
                                            </td>
                                            <td className="max-w-xs truncate px-5 py-3 text-slate-600">{log.description ?? '-'}</td>
                                            <td className="px-5 py-3 font-mono text-xs text-slate-500">{log.ip_address ?? '-'}</td>
                                            <td className="whitespace-nowrap px-5 py-3 text-slate-500">{log.created_at}</td>
                                        </tr>
                                    ))}
                                    {logs.data.length === 0 && (
                                        <tr>
                ...
                {logs.last_page > 1 && (
                    <div className="flex flex-wrap items-center justify-center gap-1">
                        {Array.from({ length: logs.last_page }, (_, i) => i + 1).map(page => (
                            <button key={page} onClick={() => router.get('/admin/audit-logs', { page, search, action: actionFilter }, { preserveScroll: true })}
...
AuditLogsIndex.layout = {
    breadcrumbs: [{ title: 'Dashboard', href: '/admin/dashboard' }, { title: 'Audit Log', href: '/admin/audit-logs' }],
};
