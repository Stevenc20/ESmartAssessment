            ...
            setQrSession({
                ...
                session: {
                    ...
                    expires_in: Math.max(
                        1,
                        Math.floor(
                            (new Date(data.session.expired_at).getTime() -
                                Date.now()) /
                                1000,
                        ),
                    ),
...
return;
        ...
        try {
            const res = await fetch(
                `/pertemuan/${qrSession.pertemuanId}/absen/status`,
            );
            const data = await res.json();
...
return;
    ...
    useEffect(() => {
        if (!qrSession || qrSession.session.expires_in > 0) {
            return;
        }
...
return null;
                ...
                return {
                    ...prev,
                    session: { ...prev.session, expires_in: remaining },
                };
            });
        }, 1000);
    ...
    const totalPertemuan = roadmaps.reduce(
        (sum, r) => sum + r.pertemuan.length,
        0,
    );
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-slate-500">
                                    Kelola sesi pertemuan per bulan
                                </p>
                            </div>
                        </div>
                        <Button
                            onClick={() => setShowAddDialog(true)}
                            className="inline-flex items-center gap-2 rounded-lg bg-sky-600 px-4 py-2.5 text-sm font-bold text-white shadow-sm transition-all hover:bg-sky-700"
                        >
                    ...
                    {years.length > 0 ? (
                            ...
                            >
                                ...
                                >
                                    ...
                                    <span className="text-xs text-slate-400">
                                        ({groupedByYear[year].length} roadmap)
                                    </span>
                                </button>
                        ...
                        <div className="flex flex-col items-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-16 text-center">
                            ...
                            <p className="text-xs text-slate-400">
                                Tambah roadmap baru untuk mulai mengelola
                                pertemuan.
                            </p>
                            <Button
                                onClick={() => setShowAddDialog(true)}
                        ...
                        <div className="flex min-w-0 flex-col items-center gap-2.5">
                            ...
                            <div className="flex w-full items-center justify-between rounded-lg bg-sky-50 px-3 py-2">
                                ...
                                <span className="text-base font-bold text-sky-700">
                                    {qrSession.total_scanned}
                                </span>
                            </div>

                            {/* Attendee list */}
                            {qrSession.attendees.length > 0 && (
                                <div className="max-h-28 w-full divide-y divide-slate-100 overflow-y-auto rounded-lg border border-slate-200">
                            ...
                            <Button
                                onClick={tutupAbsen}
                                size="sm"
                                className="w-full bg-red-500 text-xs text-white hover:bg-red-600"
                            >
            ...
            >
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Buat roadmap baru untuk satu bulan. 4 pertemuan
                            default akan otomatis dibuat.
                        </DialogDescription>
                    </DialogHeader>
                    <form
                        onSubmit={submitAddRoadmap}
                    ...
                    >
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <Select
                                    value={data.bulan}
                                    onValueChange={(v) => setData('bulan', v)}
                                >
                                    <SelectTrigger>
        ...
        >
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                <div className="sm:col-span-1">
                    <label className="mb-0.5 block text-[10px] font-semibold tracking-wider text-slate-400 uppercase">
                        Judul
                    </label>
                    <Input
                        value={data.judul}
