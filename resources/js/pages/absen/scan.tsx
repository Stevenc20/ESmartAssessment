...
export default function ScanAbsen({ status, message, pertemuan, scan_time }: Props) {
    return (
        <>
            <Head title="Absen" />
            ...
            <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-sky-50 to-indigo-50 p-4">
                <div className="w-full max-w-sm">
                    <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-lg text-center">
                        ...
                        <div className="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-full"
                            style={{
                                backgroundColor:
                                    ...
                                    status === 'already' ? '#fef3c7' :
                                    '#fee2e2'
                            }}
                        >
                            {status === 'success' && <CheckCircle className="h-10 w-10 text-emerald-600" />}
                            {status === 'already' && <Clock className="h-10 w-10 text-amber-600" />}
                            {status === 'invalid' && <XCircle className="h-10 w-10 text-red-600" />}
                        </div>
                        ...
                        {/* Pertemuan name */}
                        {pertemuan && (
                            <p className="text-sm text-slate-500 mb-4">{pertemuan}</p>
                        )}
                        ...
                        {status === 'invalid' && (
                            <div className="mt-4 flex items-center justify-center gap-1.5 text-xs text-slate-400">
                                ...
                                <span>Hubungi guru untuk informasi lebih lanjut</span>
                            </div>
                        )}
