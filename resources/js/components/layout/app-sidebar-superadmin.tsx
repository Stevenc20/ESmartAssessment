...
import { Activity, Archive, Award, BookOpen, Calendar, ClipboardList, Database, LayoutGrid, Megaphone, Monitor, Settings, Shield, ToggleLeft, Users } from 'lucide-react';
...
const sections: { label: string; items: NavItem[] }[] = [
    ...
    },
    {
        ...
        items: [
            ...
            { title: 'Pengumuman Global',   href: '/admin/global-announcements', icon: Megaphone   },
            ...
            <SidebarHeader className="px-4 py-4 group-data-[collapsible=icon]:px-2 group-data-[collapsible=icon]:py-1 transition-all duration-200">
                ...
                <div className="group-data-[collapsible=icon]:hidden px-1">
                    <div className="flex items-center gap-2">
                        <span className="h-2 w-2 rounded-full flex-shrink-0" style={{ background: '#436391' }} />
                        <span className="text-xs font-medium" style={{ color: '#436391' }}>Super Admin</span>
                    </div>
                </div>
            </SidebarHeader>
            <SidebarContent className="px-2 py-2">
                {sections.map((s) => <NavMain key={s.label} items={s.items} label={s.label} />)}
            </SidebarContent>
